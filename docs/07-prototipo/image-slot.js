...
(() => {
  ...
  const stylesheet =
    ...
    // compact: slots too small for a caption (avatars, retratos) show only the icon.
    ':host([compact]) .empty{padding:0;gap:0}' +
    ':host([compact]) .empty .cap,:host([compact]) .empty .sub{display:none}' +
    ':host([compact]) .empty svg{width:16px;height:16px;opacity:.35}' +
    ':host([compact]) .frame{background:transparent}' +
