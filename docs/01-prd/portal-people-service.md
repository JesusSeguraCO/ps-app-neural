...
| **Persistencia** | **PostgreSQL administrado** | Inventario, enlaces, sesiones, auditoría, telemetría y cola de trabajo, en una sola base |
...
| **Correo saliente** | **Mailgun**, desde `notify@people.trycore.com`, con SPF, DKIM y DMARC propios del subdominio. `trycore.com` sigue en Google Workspace, independiente. Rebotes y quejas llegan a la aplicación por aviso firmado | El código de acceso es crítico: si cae en spam, nadie entra. Un proveedor transaccional da reputación de envío y avisa de cada rebote |
...
| **Respaldo** | Respaldo diario de la base administrada con recuperación a un punto en el tiempo de 7 días, más exportación semanal cifrada a Google Drive de Trycore. Antes de producción, restauración de prueba | Un fallo de la base se recupera con minutos de pérdida; la pérdida de la cuenta o la región queda cubierta por la exportación semanal (riesgo aceptado, §10.3) |
| **Protección perimetral** | Cloudflare delante de los dos hosts, con límite de peticiones en los puntos de token y código; el servidor rechaza lo que no llega por Cloudflare | Evita la prueba masiva de tokens y que alguien se salte el borde |
...
| **Sistema operativo del hosting fuera de soporte** | *No aplica desde la v4.12.* El hosting compartido se abandonó (D-23 revisada); la plataforma administrada (§8.3) aplica los parches del sistema y la base de datos | Se mantienen: datos mínimos en servidor, secretos solo en el servidor y Cloudflare delante |
| **Pérdida de la cuenta o de la región del proveedor de nube** | La base de datos administrada tiene respaldo diario y recuperación a un punto en el tiempo de 7 días: un fallo de la base se recupera con minutos de pérdida. Si se perdiera la cuenta o la región completa, solo queda la exportación semanal cifrada a Google Drive de Trycore: hasta 7 días de inventario, consentimientos, auditoría y solicitudes en cola, y la evidencia (almacenamiento privado) sin copia fuera del proveedor | **Riesgo aceptado** por el sponsor el 2026-09-25. Las solicitudes ya enviadas a HubSpot sobreviven allí; el inventario se reconstruye desde la exportación del banco (RF-8.15.9) |
...
| 4.12 | 2026-09-25 | **D-23 revisada: cambio de plataforma.** El hosting compartido se abandona; la v1 corre en contenedores Docker portables en DigitalOcean, con Next.js en TypeScript, un proceso de trabajo diferido, PostgreSQL administrado y Mailgun. **§8.3 reescrita.** RF-9.6.1: la cola vive en la base y la reintenta el proceso de trabajo diferido. RF-8.11.1: evidencia en almacenamiento privado. RF-1.2.6, RF-2.6.4 y RF-8.1 dejan de apoyarse en el hosting. §10.3: se retira el riesgo del sistema operativo y el de pérdida del servidor pasa a pérdida de la cuenta o región del proveedor (aceptado, hasta 7 días). Pruebas en un servidor de la empresa con Docker; datos en Estados Unidos, a validar por legal. Detalle técnico en ADR 0008–0010 |
| 4.11 | 2026-09-25 | **Nueva D-24:** la búsqueda interpreta con un algoritmo propio (RF-2.6 reescrito) y **Gemini** queda acotado a requerimientos pegados largos (RF-12.2.1) y a proponer léxico con aprobación humana (RF-8.12.1). Proveedor del modelo definido: Gemini, con token personal y datos ficticios en desarrollo y llave de negocio en producción |
