...
- **PRD técnico (fuente del stack)**: `docs/01-prd/portal-people-service.md#83-plataforma-de-ejecución-contenedores-en-digitalocean` (D-23 revisada 2026-09-25, PRD v4.12) + decisiones técnicas en `docs/adr/0008`–`0010`: contenedores Docker portables; producción en DigitalOcean App Platform (región EE. UU.), staging en un servidor de la empresa con Docker; Next.js 15.5+ TypeScript (dos apps: portal y panel) + worker Node; PostgreSQL 16 administrado; Mailgun. `docs/01-prd/requisitos-tecnicos-hosting.md` es solo antecedente (cPanel abandonado). Sin proveedor de identidad
- **Capa de servicios externos / IA (frontera)**: tres fronteras, todas solo server-side (adaptadores en `packages/infra`, `fetch` con timeout): **HubSpot API** (RF-9 negocio en pipeline People Service, contacto/empresa sin duplicar, timeline, cola de reintento RF-9.6; RF-18 correo curado) y **Gemini** (D-24: la búsqueda interpreta con algoritmo propio; Gemini solo extrae criterios de requerimientos pegados largos y propone léxico con aprobación humana; nunca ve ni redacta sobre perfiles, RF-16; desarrollo con token personal y solo datos ficticios, producción con llave de negocio). Tercera: **Mailgun** (API HTTP desde `notify@people.trycore.com`) para códigos de acceso de un uso, avisos y boletín, con eventos por webhook firmado. **No hay proveedor de identidad** (ni Keycloak ni similar, decisión del sponsor 2026-09-24): el acceso es enlace con token + lista nominal de correos invitados + código al buzón (reenviar no da acceso); panel: @trycore.com inscrito + código + sesión de una jornada. Declaradas en `.claude/config/build-config.json#boundaries`
- **Lógica que debe ser determinista (no-IA)**: intérprete de consultas (RF-2.6: léxico, normalización, distancia de edición, patrones) y motor único de criterios (RF-13.8: obligatorio/deseable, conteos del panel = resultados, «lo más cercano» = falla exactamente un obligatorio, evidencia ✓/– por criterio RF-13.10, sin porcentajes); reglas del panel (consentimiento bloquea publicar, coherencia estado/disponibilidad RF-8.14, catálogos sin borrado); importación masiva (modos, fusión, `null` explícito, sin publicar ni conceder consentimiento, reversión); enlaces curados (firma, vigencia, revocación, re-evaluación al abrir RF-19); escalamiento 4 h / 24 h hábiles RF-9.7.3
- **Categorías de datos sensibles / PII reguladas**: Ley 1581/2012 (habeas data). Profesionales: nombre + primer apellido, trayectoria, clientes nombrados, ciudad, consentimiento nominal. Lista negra B.4 que nunca cruza al portal: foto, datos de contacto, CV, motivación/proyección, DISC detallado, promedio/certificaciones. Artefactos de evidencia (documento, transcripción, repositorio; en almacenamiento privado DO Spaces) nunca expuestos a la cara cliente. Clientes: correo corporativo, identidad del contacto, telemetría atribuida. Al modelo jamás datos de perfiles (RF-16.2)
- **Secretos server-side**: `HUBSPOT_PRIVATE_APP_TOKEN`, `GEMINI_API_KEY`, `MAILGUN_API_KEY`, `MAILGUN_WEBHOOK_SIGNING_KEY`, `LINK_SIGNING_SECRET` (firma de enlaces curados/tokens de cuenta), `OTP_PEPPER` (códigos de un uso), `EMAIL_HMAC_KEY`, `AUDIT_HMAC_KEY`, `EDGE_SECRET` (cabecera de borde de Cloudflare), `SPACES_KEY`/`SPACES_SECRET`, `LATIDO_URL`, `DATABASE_URL` por componente (roles `ps_portal`/`ps_panel`/`ps_worker`/`ps_migrador`, nunca `doadmin`). Viven como variables `SECRET` de App Platform a nivel de componente (ADR-0010); nunca en el repositorio ni en la imagen
